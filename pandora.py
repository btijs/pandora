...
pandora_state = {"only_reporting": False, "in_execution": False}
...
def pandora_setup(pandora_ctx: PandoraContext, binary_path: Path):
    ...
    with Live(console_progress, console=console):
        ...
        if pandora_state["ctx"].with_cfg:
            ...
            task_simplify = console_progress.add_task(description="Simplifying CFG to dag of enclave entry tree", total=None)
        ...
        if pandora_state["ctx"].with_cfg:
            ...
            pandora_state["cfg_nodes"] = cfg.simplify_cfg_to_tree(my_cfg.get_any_node(explorer.BasicBlockExplorer().get_init_state().addr))
...
def pandora_cfg(pandora_ctx: PandoraContext, binary_path: Path):
    ...
    """
    console.print(log_format.format_header("Pandora CFG Generator. CAREFUL: This may take a VERY long time and may not even work depending on the binary. Only use it for troubleshooting Pandora on very small executables."))
...
def main_callback(
    ...
    report_level: LogLevel = typer.Option("info", "-L", "--report-level", help="The level for pandora reports. Set to debug to get all information.", rich_help_panel="Report generation"),
